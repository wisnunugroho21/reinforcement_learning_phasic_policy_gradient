...
class Agent():  
    ...
    # Loss for PPO  
    def compute_policy_loss(self, action_probs, values, old_action_probs, old_values, next_values, actions, rewards, dones):
    ...
    def compute_aux_loss(self, action_probs, values, old_action_probs, Returns):
        # Don't use old value in backpropagation
        Old_action_probs    = old_action_probs.detach()
    ...
    def training_ppo(self, states, actions, rewards, dones, next_states):
        ...
        loss                = self.compute_policy_loss(action_probs, values, old_action_probs, old_values, next_values, actions, rewards, dones)
    ...
    def training_aux(self, states):
        ...
        joint_loss                      = self.compute_aux_loss(action_probs, values, old_action_probs, Returns)
...
def main():
    ...
    n_update            = 256 # How many episode before you update the Policy. Recommended set to 128 for Discrete
    ...
    value_clip          = 4.0 # How many value will be clipped. Recommended set to the highest or lowest possible reward
    entropy_coef        = 0.05 # How much randomness of action you will get
    vf_loss_coef        = 1.0 # Just set to 1
    batchsize           = 32 # How many batch per update. size of batch = n_update / batchsize. Recommended set to 4 for Discrete
